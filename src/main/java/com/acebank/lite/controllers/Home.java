    ...
    public class Home extends HttpServlet {
        ...
        protected void doPost(HttpServletRequest request, HttpServletResponse response)
            ...
            String redirectUrl = request.getContextPath() + "/home"; // Default redirect back to dashboard
            ...
            try {
                ...
                if (depositAmtStr != null && !depositAmtStr.trim().isEmpty()) {
                    ...
                    if (status) {
                        session.setAttribute("loanMessage", "Deposit Successful");
                        session.setAttribute("loanMessageType", "success");
                    } else {
                        session.setAttribute("loanMessage", "Deposit Failed");
    ...
    // --- ACTION 2: WITHDRAW ---
                    ...
                    if ("SUCCESS".equals(status)) {
                        session.setAttribute("loanMessage", "Withdrawal Successful");
                        session.setAttribute("loanMessageType", "success");
                    } else {
                        session.setAttribute("loanMessage", status);
                        session.setAttribute("loanMessageType", "error");
                    }
                ...
                // --- ACTION 3: TRANSFER ---
                    ...
                    ServiceResponse resp = bankService.processTransfer(accountNumber, recipientAcc, amount);
                    if (resp.success()) {
                        session.setAttribute("loanMessage", resp.message());
                        session.setAttribute("loanMessageType", "success");
                    } else {
                        session.setAttribute("loanMessage", resp.message());
                        session.setAttribute("loanMessageType", "error");
                ...
                session.setAttribute("loanMessage", "Invalid Amount Format");
                session.setAttribute("loanMessageType", "error");
            } catch (Exception e) {
                log.severe("Transaction Error: " + e.getMessage());
                session.setAttribute("loanMessage", "Transaction could not be completed");
                session.setAttribute("loanMessageType", "error");
            }
